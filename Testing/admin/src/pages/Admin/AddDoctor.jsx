...
const AddDoctor = () => {
    ...
    // Function to refresh frontend doctor list
    const refreshFrontendDoctors = async () => {
        try {
            // Make a GET request to the frontend's doctor list endpoint
            await axios.get(backendUrl + '/api/doctor/list')
            
            // Send a message to the frontend window if it exists
            if (window.opener) {
                window.opener.postMessage('REFRESH_DOCTORS', '*')
            }
        } catch (error) {
            console.log('Error refreshing frontend:', error)
        }
    }
    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // After successfully adding doctor, refresh frontend
                await refreshFrontendDoctors()
            } else {
